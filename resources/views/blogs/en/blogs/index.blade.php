...
@section('titulo', 'Blogs en Inglés')

@section('contenido')
<div class="pm-page-header">
    <div>
        <h1>Blogs en Inglés</h1>
        <small>Artículos del blog publicados en inglés</small>
    </div>
    <a href="{{ route('enblogs.create') }}" class="btn btn-pm-primary btn-sm">
            ...
            <table id="tabladatos" class="table pm-table table-hover mb-0">
                <thead>
                    <tr>
                        ...
                        <th>Imagen</th>
                        <th>Tags</th>
                        <th class="sortable" data-index="4">Slug</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="font-size:0.82rem;">
                            @if ($blog->blog)
                                <a href="{{ route('blogs.edit', $blog->blog->id) }}">{{ Str::limit($blog->blog->nombre, 40) }}</a>
                            @else
                                <span class="text-muted">—</span>
                            @endif
                        </td>
                        <td><img src="{{ asset($blog->img) }}" width="100" height="65" style="border-radius:8px; object-fit:cover;" alt="{{ $blog->nombre }}"></td>
                        <td>
                        ...
                        <td class="text-center" style="white-space:nowrap;">
                            <form action="{{ route('enblogs.destroy', $blog->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <a href="{{ route('enblog', $blog->slug) }}" class="btn btn-action btn-action-view" title="Ver en web" target="_blank">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ route('enblogs.edit', $blog->id) }}" class="btn btn-action btn-action-edit" title="Editar">
