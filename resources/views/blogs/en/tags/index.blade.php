...
@section('titulo', 'Tags en Inglés')

@section('contenido')
<div class="pm-page-header">
    <div>
        <h1>Tags en Inglés</h1>
        <small>Etiquetas para categorizar los blogs en inglés</small>
    </div>
    <a href="{{ route('entags.create') }}" class="btn btn-pm-primary btn-sm">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center" style="white-space:nowrap;">
                            <form action="{{ route('entags.destroy', $tag->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <a href="{{ route('entag', $tag->slug) }}" class="btn btn-action btn-action-view" title="Ver en web" target="_blank">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ route('entags.edit', $tag->id) }}" class="btn btn-action btn-action-edit" title="Editar">
