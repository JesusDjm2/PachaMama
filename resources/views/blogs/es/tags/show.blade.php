...
@section('content')
            ...
            <div class="row text-center">
                <div class="col-lg-12">
                    ...
                    <p class="text-center" style="color: #fff">
                        Blogs relacionados con {{ $tag->nombre }}: {{ $coincidencias }} artículos encontrados
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <div class="col-lg-3 col-12 sidebar-col">
                    <div class="sticky-sidebar">
                        <div class="sidebar-header d-flex align-items-center justify-content-between">
                            <h3 class="mb-0">Todas las etiquetas</h3>
                            <button class="sidebar-toggle d-lg-none" id="sidebarToggle"
                                aria-expanded="false" aria-label="Mostrar u ocultar etiquetas">
                        ...
                        <div class="tags-list" id="tagsList">
                            @foreach ($allTags as $sidebarTag)
                                <div class="tag-item {{ $sidebarTag->id === $tag->id ? 'active' : '' }}">
                                    <a href="{{ route('tag', $sidebarTag->slug) }}" class="tag-link">
                                        <i class="fas fa-tag fa-xs"></i>
                                        <span class="tag-name">{{ $sidebarTag->nombre }}</span>
                                        <span class="badge badge-count">{{ $sidebarTag->blogs->count() }}</span>
                                    </a>
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="col-lg-9">
                    <div class="row">
                        <div class="col-12">
                            <h2 class="mb-4 w-100">Blogs etiquetados con "{{ $tag->nombre }}"</h2>
                        </div>
                        <div class="col-12">
                            <div id="blogContainer" class="row">
                                @foreach ($tag->blogs as $blog)
                                    <div class="blog-item col-lg-4 col-md-6 mb-4"
                                        data-title="{{ strtolower($blog->nombre) }}"
                                        data-description="{{ strtolower($blog->descripcion) }}">
                                        <div class="card card-new">
                                            ...
                                            <div class="card-body text-center d-flex flex-column">
                                                ...
                                                <p class="text-card flex-grow-1 blog-description mb-2">
                                                    {{ $blog->descripcion }}
                                                </p>
                                                <div class="tags mb-1">
                                                    @foreach ($blog->tags as $blogTag)
                                                        <a
                                                            href="{{ route('tag', $blogTag->slug) }}">#{{ $blogTag->nombre }}</a>
                                                    @endforeach
                                                </div>
                                                <a href="{{ route('blog.show', ['id' => $blog->id, 'slug' => $blog->slug]) }}"
                                                    class="btn-blog-card mt-auto">
                            ...
                            @if ($tag->blogs->isEmpty())
                                <div class="text-center py-5">
                                    <p class="text-muted">No se encontraron blogs con esta etiqueta.</p>
                                    <a href="{{ route('listado') }}" class="btn btn-pm-primary">Ver todos los blogs</a>
                                </div>
                            @endif
                            <div id="noResults" class="text-center py-5" style="display: none;">
                                <p class="text-muted">No se encontraron blogs que coincidan con tu búsqueda.</p>
                                <button class="btn btn-outline-secondary" onclick="clearSearch()">Limpiar
                                    búsqueda</button>
                            </div>
                        </div>
    ...
    <style>
        ...
        @media (max-width: 991.98px) {
            ...
            /* Collapsar tags en tablet/móvil */
            .tags-list {
                overflow: hidden;
                max-height: 0;
